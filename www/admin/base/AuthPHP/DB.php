...
trait DB{
	...
	private function _db_insertToken($token, $id_user){
		...
		$timeDB = $this->_db_get('SELECT NOW() as now');
		$time = $timeDB ? $timeDB[0]["now"] : date("Y-m-d H:i:s");

		return $this->_db_set('INSERT INTO '.$this->tUserToken.
							  ' ('.$this->fToken.', '.$this->fIdUser.', '.$this->fTokenIp.', '.$this->fTokenAdd.') 
							  VALUES (?, ?, ?, ?)',
							  array( $token, (int)$id_user, $_SERVER['REMOTE_ADDR'], $time ));
	}
